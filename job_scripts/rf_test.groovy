...
def stepCheckoutRobotFWTests() {
    helperCheckoutRepo(params.ROBOTFW_URL,
                       params.ROBOTFW_PR,
...
def stepCheckoutRobotFWFrontend() {
    if (params.GENERATE_HTML) {
        helperCheckoutRepo("https://github.com/RIOT-OS/RobotFW-frontend.git",
                           "",
                           "main",
                           "RobotFW-frontend")
    }
...
def stepCheckoutRIOT() {
    helperCheckoutRepo(params.RIOT_URL,
                       params.RIOT_PR,
                       params.RIOT_BRANCH,
                       "RIOT",
                       params.RIOT_OWNER,
...
def stepFillBoardTestQueue() {
    nodeBoards = getBoardsFromNodes(params.HIL_BOARDS)
    tests = getTests(params.HIL_TESTS)
...
def stepCompileResults() {
    compileResults(params.GENERATE_HTML)
}

def stepNotify() {
    if (params.ROBOTFW_PR != "") {
        msg = generateNotifyMsgMD(totalResults)
        notifyOnPR(params.ROBOTFW_OWNER, "RobotFW-tests", params.ROBOTFW_PR, msg)
    }
    if (params.RIOT_PR != "") {
        msg = generateNotifyMsgMD(totalResults)
        notifyOnPR(params.RIOT_OWNER, "RIOT", params.RIOT_PR, msg)
    }
}
...
def stepBuildJobs() {
    buildJobs(boardTestQueue, totalResults, params.EXTRA_MAKE_COMMANDS)
}
